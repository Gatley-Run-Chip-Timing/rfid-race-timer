...
void loop() {
  if (uploadMode == 1) {
    ...
    uint8_t dataToWrite[4] = {(uploadIndex & 0x0000ff00) >> 8, uploadIndex & 0x000000ff};
  ...
  if (mode == 2) {
    ...
    if (uhf.readCard(readBuffer, sizeof(readBuffer), 0x04, 0, 0x00000000)) {
      ...
      if (good == 1) {
        ...
        String json = "{\"key\": \"" + API_KEY +  "\", \"position\": \"" + position + "\", \"unixEpoch\": " + String(epochTime) + ", \"rfidId\": \"" + bufferHere + "\"}";
